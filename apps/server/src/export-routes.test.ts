...
async function fixture() {
  ...
  db.prepare("INSERT INTO books (id,title,original_file_path,original_file_hash,encoding,import_status) VALUES ('book','book','books/source.txt',?,'UTF-8','ready')").run("1".repeat(64));
  ...
  db.prepare(`INSERT INTO audio_segments (timeline_hash,segment_index,episode_id,script_version_id,text,provider_id,voice,rate,input_hash,relative_path,file_hash,bytes,duration_ms,created_at)
    VALUES (?,0,'episode','script','text','test','voice',0,?,'episodes/episode/audio/segments/audio.wav',?,1,60000,1)`).run(TIMELINE, "5".repeat(64), "6".repeat(64));
  ...
  return { app, connection, dataRoot, exported };
}
