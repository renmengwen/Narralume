...
function spawnChild(dataRoot: string, mode: "crash" | "recover") {
  ...
  return new Promise<{ code: number | null; stdout: string; stderr: string }>((resolve, reject) => {
    const child = spawn(process.execPath, [...process.execArgv, script], {
      env: {
        ...process.env,
        NARRALUME_P2_CHECKPOINT_CHILD: mode,
        NARRALUME_P2_CHECKPOINT_DATA_ROOT: dataRoot,
      },
...
const childMode = process.env.NARRALUME_P2_CHECKPOINT_CHILD;
const childDataRoot = process.env.NARRALUME_P2_CHECKPOINT_DATA_ROOT;
