...
const expectedSha256 = (process.env.NARRALUME_LONG_TEXT_SHA256 ?? FROZEN_SOURCE_SHA256).toLowerCase();
...
let sourcePath = process.env.NARRALUME_LONG_TEXT_PATH;
...
try {
  ...
  const memoryLimitMiB = Number(process.env.NARRALUME_MEMORY_DELTA_MIB ?? 96);
  assert(Number.isFinite(memoryLimitMiB) && memoryLimitMiB > 0, "NARRALUME_MEMORY_DELTA_MIB 必须是正数");
  ...
  if (process.env.NARRALUME_GATE_FAULT_AFTER_SAMPLER === "1") {
