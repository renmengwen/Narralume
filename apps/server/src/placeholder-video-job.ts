...
async function runProcess(command: string, args: string[], options: { cwd?: string; signal?: AbortSignal } = {}) {
  ...
  if (code !== 0) throw new Error(`${command} 执行失败${stderr.trim() ? `：${stderr.trim()}` : ""}`);
  return stdout;
}

export async function probePlaceholderVideo(path: string, signal?: AbortSignal): Promise<VideoProbe> {
  ...
  if (video.length !== 1 || audio.length !== 1 || !stream || stream.codec_name !== "h264" ||
      stream.width !== 1080 || stream.height !== 1920 || stream.r_frame_rate !== "25/1" ||
      !Number.isSafeInteger(durationMs) || durationMs < 1 || Number(parsed.format?.size) !== info.size) {
    throw new Error("占位视频流、尺寸、帧率、大小或时长无效");
  }
