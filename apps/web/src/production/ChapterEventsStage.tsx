      ...
      <div className="border-t border-[var(--border-subtle)]">
        ...
        {drafts.map((draft, index) => <article className="border-b border-[var(--border-subtle)] py-5" key={draft.key}>
          <div className="grid grid-cols-[120px_minmax(0,1fr)_auto] gap-3 max-md:grid-cols-1">
            <label className="text-xs text-[var(--fg-secondary)]">事件类型<select aria-label={`事件 ${index + 1} 类型`} disabled={locked} value={draft.type} onChange={(event) => updateDraft(index, { type: event.target.value as ChapterEventDraft["type"] })} className="mt-1 block w-full rounded border border-[var(--border-subtle)] bg-[var(--bg-canvas)] p-2 text-sm">{CHAPTER_EVENT_TYPE_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
            <div className="grid grid-cols-2 gap-3 max-md:grid-cols-1">
              <label className="text-xs text-[var(--fg-secondary)]">{draft.type === "causality" ? "原因" : draft.type === "revelation" ? "揭示内容" : draft.type === "suspense" ? "悬念问题" : "名称"}<input aria-label={`事件 ${index + 1} 主要内容`} disabled={locked} value={draft.primary} onChange={(event) => updateDraft(index, { primary: event.target.value })} className="mt-1 block w-full rounded border border-[var(--border-subtle)] bg-[var(--bg-canvas)] p-2 text-sm" /></label>
              {draft.type === "causality" || draft.type === "character" || draft.type === "location" || draft.type === "prop" ? <label className="text-xs text-[var(--fg-secondary)]">{draft.type === "causality" ? "结果" : "详情（可选）"}<input aria-label={`事件 ${index + 1} 次要内容`} disabled={locked} value={draft.secondary} onChange={(event) => updateDraft(index, { secondary: event.target.value })} className="mt-1 block w-full rounded border border-[var(--border-subtle)] bg-[var(--bg-canvas)] p-2 text-sm" /></label> : null}
            </div>
            <button type="button" disabled={locked} className="self-end px-2 py-2 text-xs text-[var(--accent)] disabled:opacity-50" onClick={() => setDrafts((current) => current.filter((_, draftIndex) => draftIndex !== index))}>移除事件</button>
          </div>
          <div className="mt-3 space-y-2">{draft.sources.map((source, sourceIndex) => <div className="grid grid-cols-[1fr_1fr_auto] gap-2" key={`${draft.key}-${sourceIndex}`}>
            <label className="text-xs text-[var(--fg-secondary)]">证据起始字节<input aria-label={`事件 ${index + 1} 证据 ${sourceIndex + 1} 起始字节`} type="number" disabled={locked} value={source.byteStart} onChange={(event) => updateDraft(index, { sources: draft.sources.map((item, itemIndex) => itemIndex === sourceIndex ? { ...item, byteStart: Number(event.target.value) } : item) })} className="mt-1 block w-full rounded border border-[var(--border-subtle)] bg-[var(--bg-canvas)] p-2 font-mono text-xs" /></label>
            <label className="text-xs text-[var(--fg-secondary)]">证据结束字节<input aria-label={`事件 ${index + 1} 证据 ${sourceIndex + 1} 结束字节`} type="number" disabled={locked} value={source.byteEnd} onChange={(event) => updateDraft(index, { sources: draft.sources.map((item, itemIndex) => itemIndex === sourceIndex ? { ...item, byteEnd: Number(event.target.value) } : item) })} className="mt-1 block w-full rounded border border-[var(--border-subtle)] bg-[var(--bg-canvas)] p-2 font-mono text-xs" /></label>
            <button type="button" disabled={locked || draft.sources.length === 1} className="self-end px-2 py-2 text-xs text-[var(--accent)] disabled:opacity-50" onClick={() => updateDraft(index, { sources: draft.sources.filter((_, itemIndex) => itemIndex !== sourceIndex) })}>移除证据</button>
          </div>)}</div>
          <button type="button" disabled={locked} className="mt-2 text-xs font-semibold text-[var(--accent)] disabled:opacity-50" onClick={() => updateDraft(index, { sources: [...draft.sources, { byteStart: selected!.byte_start, byteEnd: selected!.byte_end }] })}>添加证据范围</button>
        </article>)}
        {selected && !drafts.length && !locked ? <p className="m-7 text-[13px] text-[var(--fg-tertiary)]">本章还没有结构化事件。可添加人工事件；保存空列表会清空本章已有事件。</p> : null}{!selected ? <p className="m-7 text-[13px] text-[var(--fg-tertiary)]">选择章节后查看结构化事件和原文证据。</p> : null}
      </div>
    </section>
