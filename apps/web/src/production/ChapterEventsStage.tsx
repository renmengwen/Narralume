        ...
        {drafts.map((draft, index) => <AccordionItem className="rounded border border-[var(--border-subtle)] bg-[var(--bg-canvas)]" key={draft.key} value={draft.key}>
          <AccordionTrigger>
            <span className="min-w-0">
              ...
              <span className="block truncate text-xs font-normal text-[var(--fg-tertiary)]">事件 {index + 1} · {draft.sources.length} 个证据范围{locked ? " · 已锁定" : ""}</span>
            </span>
          </AccordionTrigger>
          <AccordionContent>
          ...
          <button type="button" disabled={locked} className="mx-3 mb-3 text-xs font-semibold text-[var(--accent)] disabled:opacity-50" onClick={() => updateDraft(index, { sources: [...draft.sources, { byteStart: selected!.byte_start, byteEnd: selected!.byte_end }] })}>添加证据范围</button>
          </AccordionContent>
        </AccordionItem>)}
        </Accordion>
