...
export function AssetStage(props: Parameters<typeof useAssetWorkspace>[0]) {
  ...
  return <div className="grid min-h-[calc(100vh-344px)] grid-cols-[minmax(250px,.78fr)_minmax(480px,1.45fr)_minmax(330px,1fr)] max-md:grid-cols-1">
    ...
    <ImagePromptBuilder asset={state.selectedAsset} episode={state.episode} busy={state.busy} alias={state.aliasDraft} prompt={state.prompt} derivedFromCandidateId={state.derivedFromCandidateId} parts={state.promptParts} onAlias={state.setAliasDraft} onAddAlias={() => void state.addAlias()} onPart={state.updatePromptPart} onPrompt={state.updatePrompt} onGenerate={() => void state.generateCandidate()} />
