...
export function AudioStage(props: {
  ...
  return <div className="grid grid-cols-[280px_minmax(0,1fr)] border-t border-[var(--border-subtle)] max-xl:grid-cols-1">
    <aside className="border-r border-[var(--border-subtle)] p-5 max-xl:border-r-0 max-xl:border-b">
      ...
      <button type="button" disabled={props.busy || props.jobActive || state.approval?.status !== "approved" || !state.voice.trim() || !Number.isInteger(state.rate) || state.rate < -10 || state.rate > 10} onClick={() => void state.createTimeline()} className="mt-5 w-full rounded bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">生成语音时间轴</button>
    </aside>
