...
export function ScriptStage({
  ...
  <div className="grid grid-cols-[260px_minmax(0,1fr)_340px] border-t border-[var(--border-subtle)] max-xl:grid-cols-1">
    <aside className="border-r border-[var(--border-subtle)] p-5 max-xl:border-r-0 max-xl:border-b">
      ...
      <VersionList title="原著还原稿版本" items={faithful} onLoad={state.loadVersion} />
        ...
        {state.paragraphs.map((paragraph, index) => <article key={paragraph.key} className="rounded border border-[var(--border-subtle)] p-4">
          ...
          <Accordion type="single" collapsible className="mt-3 rounded border border-[var(--border-subtle)]">
            <AccordionItem value="sources">
              <AccordionTrigger className="font-normal"><span>引用来源 <span className="font-mono text-xs text-[var(--fg-tertiary)]">已选 {paragraph.sourceIndexes.length} / {sources.length}</span></span></AccordionTrigger>
              <AccordionContent><fieldset className="grid max-h-80 gap-2 overflow-y-auto p-3"><legend className="sr-only">段落 {index + 1} 的引用来源</legend>{sources.map((source) => <SourceOption key={source.sourceIndex} source={source} checked={paragraph.sourceIndexes.includes(source.sourceIndex)} disabled={busy} onChange={(checked) => state.updateParagraph(paragraph.key, { sourceIndexes: checked ? [...paragraph.sourceIndexes, source.sourceIndex] : paragraph.sourceIndexes.filter((item) => item !== source.sourceIndex) })} />)}{!sources.length ? <p className="text-xs text-[var(--fg-tertiary)]">当前没有可用来源；成片旁白稿只能使用对应原著还原稿的来源。</p> : null}</fieldset></AccordionContent>
            </AccordionItem>
          </Accordion>
        </article>)}
