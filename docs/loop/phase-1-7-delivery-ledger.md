...
- 当前分支：`dev`
- 当前 HEAD：`2153b9bf77d60e35cf8b755bbb93ea98c4c7c2bf`（P5-01 首版冻结控制提交；本控制提交后以 Git 历史定位最新 HEAD）
- 工作区：`dev` 仅本 Ledger 待提交；P5-01 首版冻结 candidate 已由独立 Reviewer 只读复验并判定 FAIL，tree 未变化；仅开放 `database.ts/database.test.ts` 最小修复，其他 6 个路径保持冻结。
...
- 当前 Task：`P5-01`
- 唯一下一动作：在 migration v9 增加数据库级“状态资产不能作为父资产”约束并补直接 SQL 回归；全量验证后重冻 candidate，旧 revision 不再有效。

## 当前写租约

| Task | Owner | Worktree / branch / base | 允许路径 | 状态所有权 | 排他资源 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| P5-01 | Coordinator（仅 migration 修复写者恢复写权限） | `D:\code3\Narralume-worktrees\P5-01` / `codex/p5-01` / `eb4ac1020bbff394859a0df12d7284537747f682` | 首版 8 路径；当前只开放 `apps/server/src/database.ts`、`apps/server/src/database.test.ts`，排除本 Ledger | 主资产、状态资产、系列级别名、重启恢复 | SQLite schema、Worker Git index | `changes_requested` |
...
| P5-01 资产合同 | `changes_requested` | P4-03 | Coordinator / 写租约见上 | 首版 `git-index-tree-v1:eb4ac1020bbff394859a0df12d7284537747f682:ab6cb01ed6bc3eccf6f26845e15271d40dc16285` 已失效 | `2153b9bf77d60e35cf8b755bbb93ea98c4c7c2bf` | FAIL，绑定首版 Ledger/revision：migration v9 允许直接 SQL 建立状态套状态 | FAIL，同一 finding；Store 有应用层阻断但 SQLite 持久化合同缺口 | 首版全量三门与真实 gate 均 GREEN；Reviewer 直接 SQL 探针 `master→state1→state2` 得到 `nested_state_inserted=1` | - | 仅加数据库级拒绝与直接 SQL 回归，重冻并复审 |
