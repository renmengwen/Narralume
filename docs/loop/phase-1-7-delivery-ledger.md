...
- 工作区：`dev` 仅本 Ledger 待提交；P5-02 第二版保持 13 个业务/来源路径 staged、零额外 unstaged/untracked；真实生成候选图保留在 Worker 忽略目录，`node_modules` 为临时 Junction。第二版 revision 因复审 findings 失效，尚未开放修改。
- 最近验证：2026-07-25 P5-02 第二版 Code Quality 复审 FAIL；全量 `typecheck/test/build` 与 diff 仍 GREEN，server 73/73、web 6/6；真实图片只读证据一致，未调用真实模型。
- 当前 Task：`P5-02`
- 唯一下一动作：完成本复审 finding 控制提交并推送后，只开放两项最小修复：把入队时批准稿/requestHash/provider/model 完整身份持久化进 Job payload 并在 handler 拒绝身份漂移；所有重定向响应先 cancel 再判断次数/location。重验、重冻后只复审这两个失效范围。

## 当前写租约

| Task | Owner | Worktree / branch / base | 允许路径 | 状态所有权 | 排他资源 | 状态 |
| --- | --- | --- | --- | --- | --- | --- |
| P5-02 | Coordinator | `D:\code3\Narralume-worktrees\P5-02` / `codex/p5-02` / `b0ebaea9c60c6f89b8cafef7d9038fc1a3a832ff` | 仅第二轮 findings 的最小修复路径；排除本 Ledger | 内容寻址图片、外部模型调用、候选与审核事件、Worker index | SQLite schema、候选文件目录、`app.ts`、真实 gate | `changes_requested` |
...
| P5-02 候选图与来源 | `changes_requested` | P5-01 | Coordinator / 写租约见上 | 第二版已失效：`git-index-tree-v1:b0ebaea9c60c6f89b8cafef7d9038fc1a3a832ff:00a35d28ade25fbc7a2c92259053ccf2e2f5cb48`；首版失效 | 第二版 `c54baab25852bc0a779b3e7e5766ab55ef64cb5e`；首版 `deb7f346562363c2925372a0a29a4c68d2955692` | PASS，绑定首版 Ledger/revision；继续有效 | 第二版复审 FAIL：持久 Job payload 未绑定入队时批准/requestHash/provider/model，执行时身份可漂移并重复调用；第六个 3xx 响应在抛错前未 cancel | 全量 `typecheck/test/build`、cached diff GREEN；server 73/73、web 6/6；DNS socket 绑定、生成响应有界读取、真实审核状态回读已确认关闭；未调用真实模型 | - | 只修身份漂移与末次重定向释放；重验重冻后复审这两个范围，不重复已关闭范围 |
