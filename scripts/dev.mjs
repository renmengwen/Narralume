...
// 保留单进程编排、带前缀输出和统一退出；workspace 与端口清理按 Narralume 调整。
...
if (process.argv.includes("--restart")) {
  ...
  const result = spawnSync(
    ...
    [
      "-NoProfile",
      "-Command",
      "$processIds = @(Get-NetTCPConnection -State Listen -LocalPort 5174,3101 -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique); if ($processIds.Count) { Stop-Process -Id $processIds -Force; Write-Output ($processIds -join ',') }",
    ],
    { encoding: "utf8" },
  );
...
start("api", "@narralume/server");
start("web", "@narralume/web");
console.log("开发服务已启动：前端 http://localhost:5174，后端 http://localhost:3101");
